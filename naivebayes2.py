...
# PART 2
# -----------------------------------------------------------------------------
def test_performance(pos_indicies, neg_indicies, p_pos, p_neg, size):
    ...
    for str in os.listdir(pos_review_dir):
        if ((int(str[2:5])) in pos_indicies):
            ...
            # Adding log(1 - p_pos[word]) doesn't help to improve the performance
            # so I choose not to include them. You may want to comment
            # out the code below to try it out.
            ...
            #        if 1 - p_pos[word] > 0: # avoid scaled value to be > 0.
            #            log_sum_pos += log(1 - p_pos[word]) 
            #        if 1 - p_neg[word] > 0: # avoid scaled value to be > 0.
            #            log_sum_neg += log(1 - p_neg[word]) 
...
def part2():
    ...
    test_performance(valid_pos, valid_neg, p_pos, p_neg, 100); # 100 samples for each positive and negative reviews.
    
    print("-------------------Test set performance-----------------")
    test_performance(test_pos, test_neg, p_pos, p_neg, 100); # 100 samples for each positive and negative reviews.
    
    print("-------------------Train set performance-----------------")
    test_performance(train_pos, train_neg, p_pos, p_neg, 400); # 100 samples for each positive and negative reviews.   
part2()
